...
import 'package:fluttercity/viewobject/category_map.dart';
...
class CategoryRepository extends PsRepository {
 ...
 late CategoryDao _categoryDao;
  ...
  // Future<dynamic> getCategoryList(StreamController<PsResource<List<Category>>> categoryListStream,
  //     bool isConnectedToInternet, int limit, int offset,
  ...
  //   sinkCategoryListStream(categoryListStream,
  //       await _categoryDao.getAllByMap(primaryKey, mapKey, categoryMapDao, CategoryMap(), status: status));

  //   if (isConnectedToInternet) {
  //     final PsResource<List<Category>> _resource = await _psApiService.getCategoryList(limit, offset, holder.toMap());
  ...
  //             id: data.id + paramKey, mapKey: paramKey, categoryId: data.id, sorting: i++, addedDate: '2019'));
  //       }
  ...
  // Future<dynamic> getAllCategoryList(StreamController<PsResource<List<Category>>> categoryListStream,
  //     bool isConnectedToInternet, CategoryParameterHolder holder, PsStatus status,
  //     {bool isLoadFromServer = true}) async {
  //   // Prepare Holder and Map Dao
  //   final String paramKey = holder.getParamKey();
  //   final CategoryMapDao categoryMapDao = CategoryMapDao.instance;
  ...
  //   sinkCategoryListStream(categoryListStream,
  //       await _categoryDao.getAllByMap(primaryKey, mapKey, paramKey, categoryMapDao, CategoryMap(), status: status));

  //   if (isConnectedToInternet) {
  //     final PsResource<List<Category>> _resource = await _psApiService.getAllCategoryList(holder.toMap());
  ...
  // Future<dynamic> getNextPageCategoryList(StreamController<PsResource<List<Category>>> categoryListStream,
  //     bool isConnectedToInternet, int limit, int offset, CategoryParameterHolder holder, PsStatus status,
  //     {bool isLoadFromServer = true}) async {
  //   // Prepare Holder and Map Dao
  //   final String paramKey = holder.getParamKey();
  //   final CategoryMapDao categoryMapDao = CategoryMapDao.instance;
  ...
  //           await categoryMapDao.getAll(finder: Finder(filter: Filter.equals(mapKey, paramKey)));
