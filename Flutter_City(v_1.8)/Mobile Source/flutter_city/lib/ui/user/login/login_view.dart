...
class _LoginViewState extends State<LoginView> {
  ...
  void updateCheckBox(BuildContext context, UserProvider provider) {
    if (provider.isCheckBoxSelect) {
      ...
      //   arguments:
      //       const TermsAndConditionIntentHolder(checkType: 4, description: ''),
      // );
...
class _DividerORWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final Widget _textWidget =
        Text('یا', style: Theme.of(context).textTheme.subtitle1);
    return Row(
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: PSButtonWithIconWidget(
          ...
          icon: FontAwesome.hand_pointer_o,
          colorData: widget.userProvider.isCheckBoxSelect == false
              ? PsColors.facebookLoginButtonColor
              : PsColors.facebookLoginButtonColor,
          onPressed: () {
            Fluttertoast.showToast(msg: 'فقط برای ثبت نام با ایمیل یا تلفن vpn را روشن کنید',
