...
class DiscountItemProvider extends PsProvider {
  DiscountItemProvider({@required ItemRepository? repo, int limit = 0})
      : super(repo, limit) {
    _repo = repo;

    print('DiscountItemProvider : $hashCode');
 ...
 late StreamController<PsResource<List<Item>>> itemListStream;
  ...
  void dispose() {
    ...
    subscription.cancel();
    isDispose = true;
    print('Discount Item Provider Dispose: $hashCode');
    super.dispose();
  }
  ...
  Future<void> resetDiscountItemList() async {
    isConnectedToInternet = await Utils.checkInternetConnectivity();
