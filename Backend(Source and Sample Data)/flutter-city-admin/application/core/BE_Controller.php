...
class BE_Controller extends PS_Controller {
	 ...
	 */
	function load_template( $view = false, $data = false, $is_citys = false, $load_side_menus = true ) 
	{
		// load header
		...
		if ( !empty( $view )){
			if($is_citys) {
				$this->load_view( 'partials/city_structure', array( 'view' => $view, 'data' => $data ));
			} else {
	...
	function citylist()
	{
		$this->city_list_view( $this->module_site_url( 'citylist' ));
	}

	/**
	 * Index
	*/
	function att_header_index($item_id=false)
	{
		$this->att_header_list_view( $this->module_site_url( 'index' ),$item_id);
	}

	/**
	 * Index
	 */
	function att_detail_index($id=false, $item_id=false)
	{
		$this->att_detail_list_view( $this->module_site_url( 'index' ), $id,$item_id);
	}
	...
	{
		$this->att_header_search( $this->module_site_url( 'search' ));	
	}

	/**
	 * Search
	*/
	function att_detail_search()
	{
		$this->attribute_detail_search( $this->module_site_url( 'search' ));	
	}
	 ...
	 * @param      <type>  $base_url  The base url
	 */
	function city_list_view( $base_url )
	{
		// pagination
		$rows_count = $this->data['rows_count'];
		$this->load_pag( $base_url, $rows_count );
	 ...
	 * @param      <type>  $base_url  The base url
	 */
	function att_header_list_view( $base_url , $item_id=false)
	{
		// pagination
		$rows_count = $this->data['rows_count'];
		$this->load_pag( $base_url, $rows_count );
	 ...
	 * @param      <type>  $base_url  The base url
	 */
	function att_detail_list_view( $base_url , $id=false, $item_id=false)
	{
		// pagination
		$rows_count = $this->data['rows_count'];
		$this->load_pag( $base_url, $rows_count );
	 ...
	 * @param      <type>  $base_url  The base url
	 */
	function att_header_search( $base_url )
	{
		// pagination
		$rows_count = $this->data['rows_count'];
		$this->load_pag( $base_url, $rows_count );
	 ...
	 * @param      <type>  $base_url  The base url
	 */
	function attribute_detail_search( $base_url )
	{
		// pagination
		$rows_count = $this->data['rows_count'];
		$this->load_pag( $base_url, $rows_count );
	 ...
	 * Add a new record
	 */
	function add_att_header($item_id = 0)
	{
	 ...
	 * Add a new record
	 */
	function detail_add($header_id = 0,$item_id = 0)
	{
		...
		// if the method is post
			...
			if ( $this->is_valid_input()) {
				// save user info
				$this->save(false,$header_id,$item_id);
	 ...
	 * Edit the exiting record
	 */
	function att_edit( $id ,$item_id = 0 )
	{
	 ...
	 * Edit the exiting record
	 */
	function detail_edit( $id, $header_id , $item_id)
	{
		// check access
		$this->check_access( EDIT );
		...
		// if the method is post
			...
			if ( $this->is_valid_input( $id )) {
				...
				// save user info
				$this->save( $id, $header_id, $item_id);
			}
		}
	...
	/**
	 * Edit the exiting pending item record
	 */
	function pendingedit( $id )
	{
		...
		// load entry form
		$this->load_template( 'pendings/entry_form',$this->data, true );
	}

	/**
	 * Edit the exiting reject item record
	 */
