...
function updateDataTableSelectAllCtrl(table){
	...
	function runAfterJQ() {
		...
		var rows_selected = [<?php 
			...
			$dis_itm = $this->Itemcollection->get_all_by($conds)->result();
			
			$oldchkval = "";
			foreach ($dis_itm as $pd)
			{
					$oldchkval = $oldchkval . $pd->item_id . ",";
		...
		var table = $('#item-table').DataTable({
			 "pageLength": 20,
		      'ajax':'<?php echo site_url('/admin/items/get_all_items_for_collection/') .  $collection_id ?>',
		      'columnDefs': [{
		         'targets': 0,
		         'searchable':false,
		         'orderable':false,
	   ...
	   $('#item-table tbody').on('click', 'input[type="checkbox"]', function(e){
	   ...
	   $('#item-table').on('click', 'tbody td, thead th:first-child', function(e){
	   ...
	   $('thead input[name="select_all"]', table.table().container()).on('click', function(e){
	      if(this.checked){
	         $('#item-table tbody input[type="checkbox"]:not(:checked)').trigger('click');
	      } else {
	         $('#item-table tbody input[type="checkbox"]:checked').trigger('click');
	      }
