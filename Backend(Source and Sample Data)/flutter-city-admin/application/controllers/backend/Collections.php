...
class Collections extends BE_Controller
{
	...
	function add() {
		...
		// get Item
		$this->data['itms_list'] = $this->Item->get_all_by( $conds, 40, $this->uri->segment( 4 ) );
	...
	*/	
	// item check count
	$data['itmcheck'] = explode(",", $this->get_data( 'newchkval' ));
	...
	if($data['itmcheck'][1] != "") {
		...
		if ( ! $this->check_trans()) {
			...
			// set flash error message
			$this->set_flash_msg( 'error', get_msg( 'please_select_item' ) );
				if ( !$id ){
	...
	*/
	...
	// get Product
	$this->data['prds_list'] = $this->Item->get_all_by( $conds, 40, $this->uri->segment( 5 ) );
	...
	function delete( $collection_id ) {
		...
		//Need to item collection table delete by id
		$conds['collection_id'] = $collection_id;
		if ( !$this->Itemcollection->delete_by( $conds )) {
			// rollback
			$this->db->trans_rollback();
	...
	function delete_all( $collection_id = 0 )
	{
		...
		if ( $this->ps_delete->delete_itm_collection( $collection_id, $enable_trigger ) ) {
