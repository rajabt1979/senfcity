...
class Tags extends BE_Controller {
	...
	function index() {
		...
		// load index logic
		$this->load_template( 'tags/list', $this->data, true );
	}
	...
	function search() {
		...
		// breadcrumb urls
		$this->data['action_title'] = get_msg( 'tag_search' );
	...
	function save( $id = false ) {
		 ...
		 */
		if ( !$id ) {
			if ( ! $this->insert_icon_images( $_FILES, 'tag', $data['id'], "cover" )) {
			...
			if ( ! $this->insert_icon_images( $_FILES, 'tag-icon', $data['id'], "icon" )) {
				// if error in saving image
	...
	*/
	function edit( $id ) 
	{
		...
		// breadcrumb urls
		$this->data['action_title'] = get_msg( 'tag_edit' );
...
/**
	...
	function delete( $id ) {
	    ...
	    // enable trigger to delete all Items related data
	    $enable_trigger = true;
